...
aptStuff() {
  ...
  # PACKAGES:
  # hardinfo - a system profiler and benchmark graphical tool
  ...
  # portsentry - a program that tries to detect portscans on network interfaces with the ability to detect stealth scans
  # lynis - performs an extensive health scan of your systems to support system hardening and compliance testing.
  # clamav - used to detect trojans and malicious softwares including viruses
...
firewall(){
  ...
  ufw status verbose # output firewall status in verbose (full detail) mode
}
...
if [ "$(id -u)" != "0" ]; then
  echo "You are not running auto.sh as root."
  echo "Run as 'sudo bash auto.sh'"
  exit
else
  # comment out any functions which error out, then run again.
  aptStuff
  firewall
  ipt
  configFix
